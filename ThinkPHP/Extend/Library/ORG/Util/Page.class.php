...
class Page {
    ...
    public function show($ajax_PageBox='') {
        ...
        $p = $this->varPage; $fix = C('URL_HTML_SUFFIX');
        $nowCoolPage = ceil($this->nowPage/$this->rollPage);
        //awen edit  /Message  /news/?p=1  or /index.php/Article/view/id/12/?p=3
        if(empty($this->parameter) && !empty($_REQUEST)){	
        	$this->parameter = C('SITE_URL');		
        	unset($_REQUEST['__hash__'], $_REQUEST['_URL_']);
			if($_SERVER['HTTP_X_REWRITE_URL']){
				$arr = explode('?', $_SERVER['HTTP_X_REWRITE_URL']);
				$this->parameter .= $arr[0];
			}else{
				$this->parameter .= $_SERVER['PATH_INFO'];
			}			
        	...
        	foreach($_REQUEST as $key=>$val){
				...
				if(!is_numeric($key) && $val!=''){
					if($key==$p) continue; //排除分页
					$str = "$key/".( is_array($val) ? serialize($val) : htmlspecialchars(trim($val)) ).'/';
					if(false===stripos($this->parameter.'/', $str)){
						$this->parameter .= $str;
					} 
				}
        	}
        	...
        	//结尾的处理
			if( $fix!='' && preg_match('/'.preg_quote($fix).'($|[\?&])/i',$_SERVER['REQUEST_URI']) ){ //包含.html
				$this->parameter = rtrim($this->parameter,'/').$fix;
			}else{
