<?php if (!defined('THINK_PATH')) exit();?><div class="page">
	...
	<form method="post" action="__URL__/update/navTabId/__MODULE__" class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this,navTabAjaxDone);">
		...
		<div class="pageFormContent" layoutH="10">
			...
			<div class="unit">
				<label>所属分类：</label>
                <input type="text" class="required" size="40" readonly="readonly" id="categoryName_edit_download" name="categoryName" value="<?php echo (getcategoryname($vo["tid"])); ?>"/>
                <a class="unit btn" href="__APP__/Ui/tree/selparent/0/selmore/1/cid/tid_edit_download/cname/categoryName_edit_download" target="dialog" rel="tree" mask="true" title="选择分类" width="600" height="650">选择分类</a>
                <input type="hidden" class="required" id="tid_edit_download" name="tid" value="<?php echo ($vo["tid"]); ?>" />
            ...
            <div class="unit">
                <label>预览图片：</label>
                <a href="<?php echo (chimg($vo["img"],download)); ?>" target="_blank"><img src="<?php echo (chimg($vo["img"],download,100,100)); ?>" height="40" border="0" /></a><input type="hidden" name="img" value="<?php echo ($vo["img"]); ?>" />
            ...
            <div class="unit">
				...
				<span style="color:red">上传或外部必填一项</span>
            </div>	
            <div class="unit">
                <label>外部预览图片：</label>
			...
			<div class="unit">
				<textarea class="editor required" name="content" height="350" tools="full" pagetag upLinkUrl="__APP__/download/upload/" upImgUrl="__APP__/download/upload/" upFlashUrl="__APP__/download/upload/" upMediaUrl="__APP__/download/upload/"><?php echo (htmlspecialchars($vo["content"])); ?></textarea>
			</div>
			<div class="unit">
				<label>软件版本：</label>
				<input type="text" name="rjbb" size="40" value="<?php echo ($vo["rjbb"]); ?>" class="required">
			</div>
		</div>
		<div class="formBar">
			<ul>
