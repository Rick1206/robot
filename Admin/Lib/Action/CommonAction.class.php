...
//			echo($sql);
	...
	public function forbid() {
		...
		$id = $_REQUEST [$pk];
		$condition = array ($pk => array ('in', explode(',',$id) ) );
		$field = $_REQUEST['field'] ? $_REQUEST['field'] : ($name=='Category' ? 'classstatus' : 'status');
		if ($model->forbid ( $condition, $field )!==false) {
			$this->assign ( "jumpUrl", $this->getReturnUrl () );
			$this->success ( '状态禁用成功！' );
		} else {
			$this->error  (  '状态禁用失败！' );
	...
	public function checkPass() {
		...
		$condition = array ($pk => array ('in', explode(',',$id) ) );
		$field = $_REQUEST['field'] ? $_REQUEST['field'] : ($name=='Category' ? 'classstatus' : 'status');
		if (false !== $model->checkPass( $condition, $field )) {
			$this->assign ( "jumpUrl", $this->getReturnUrl () );
			$this->success ( '状态批准成功！' );
		} else {
			$this->error  (  '状态批准失败！' );
	...
	public function recycle() {
		...
		$condition = array ($pk => array ('in', explode(',',$id) ) );
		$field = $_REQUEST['field'] ? $_REQUEST['field'] : ($name=='Category' ? 'classstatus' : 'status');
		if (false !== $model->recycle ( $condition, $field )) {
			$this->assign ( "jumpUrl", $this->getReturnUrl () );
			$this->success ( '状态还原成功！' );
		...
		if( !empty($content) && preg_match_all('/(https?:\/\/[^\'"<>\(\);\s]+?)([^\/\s\'\"=<>\(\);:\*\|]+?)\.('.str_replace(array(',',"\s"),'|',C('REMOTE_ALLOWEXTS')).')/i',$content,$remote_files) ){
			...
			foreach( $remote_files[0] as $k=>$files ){
				...
				if( is_file($_SERVER['DOCUMENT_ROOT'].$savebase.$localFileName) ){//存在则跳过，只做替换输出
					if( preg_match('/^jpeg|jpg|png|gif$/i', $remote_files[3][$k]) ) $firstImg = $basedir.$localFileName;
...
//			dump($_POST);exit;		
	...
	function do_upload(&$model){
		...
		if(!empty($upload_arr)){
			...
			$upload->allowExts  = explode(',','zip,rar,txt,doc,docx,ppt,xls,xlsx,csv,jpg,jpeg,gif,png,bmp,swf,flv,fla,avi,wmv,wma,rm,mov,mpg,rmvb,3gp,mp4,mp3,pdf');
			...
			if(!$upload->upload()) {
				...
				foreach($upload_arr as $k=>$v){
					$model->$k = __ROOT__.ltrim($upload->savePath,'.').$files[$count]['savename'];//awen 2013-07
					$count++;
				}
			}
		}
		foreach($_FILES as $k=>$v){//上传优先，兼容外部文件地址
			...
			}elseif( false!==stripos(ACTION_NAME,'insert') && $model->$k ){
				//自动水印 2013-07
				if( is_numeric($_POST['WaterConfig']) ){
					if( !isset($rs) ) $rs = M('water')->where(array('id'=>$_POST['WaterConfig']))->find();
					if( !empty($rs) ){
						import('ORG.Util.Water');//原图水印
						$wData = array(
							'waterType'=>1,
							'srcImg'=>$_SERVER['DOCUMENT_ROOT'].$model->$k,
							'pos'=>$rs['ppos'],
							'transparent'=>$rs['ptrans'],
							'waterImg'=>$_SERVER['DOCUMENT_ROOT'].chimg($rs['pimg'],'water'),
						);
						Water::output($wData);
					}
				}				
	...
	public function upload() {
		...
		$upext='zip,rar,txt,doc,docx,ppt,xls,xlsx,csv,jpg,jpeg,gif,png,bmp,swf,flv,fla,avi,wmv,wma,rm,mov,mpg,rmvb,3gp,mp4,mp3,pdf';//上传扩展名
		...
		if(isset($_SERVER['HTTP_CONTENT_DISPOSITION']))//HTML5上传
		{
			...
			{
				$temp_name=ini_get("upload_tmp_dir"); //awen 2013-07
				$temp_name=is_dir($temp_name) ? $temp_name : $attachdir;
				$temp_name.='/'.date("YmdHis").mt_rand(1000,9999).'.tmp';
		...
		if(!isset($upfile)){
			...
			if(preg_match('/'.str_replace(',','|',$upext).'/i',$extension))
			{
				...
				else
				{
					...
					$localname=preg_replace("/([\\\\\/'])/",'\\\$1',$upfile['name']);
					if($immediate=='1')$target='!'.$target;
					if($msgtype==1)$msg="'$target'";
					else $msg="{'url':'".$target."','localname':'".$localname."','id':'1'}";
					
					//自动水印 2013-07
					if( is_numeric($_POST['WaterConfig']) ){
						$rs = M('water')->where(array('id'=>$_POST['WaterConfig']))->find();
						if( $rs ){
							import('ORG.Util.Water');//原图水印
							$wData = array(
								'waterType'=>1,
								'srcImg'=>$_SERVER['DOCUMENT_ROOT'].$target,
								'pos'=>$rs['ppos'],
								'transparent'=>$rs['ptrans'],
								'waterImg'=>$_SERVER['DOCUMENT_ROOT'].chimg($rs['pimg'],'water'),
							);
