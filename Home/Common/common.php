...
function getSearchMap($fieldMap){
	...
	if(is_array($fieldMap) && !empty($fieldMap)){
		$_search_likes = $_REQUEST ['_search_likes']; $arr_search_likes = ($_search_likes!='*' && $_search_likes!='') ? explode(',',$_search_likes) : array();
		foreach ( $fieldMap as $key => $val ) {
			$mapkey = is_numeric($key) ? $val : $key;
			$is_like = ($_search_likes=='*' || in_array($mapkey, $arr_search_likes));

			if(($keyword=htmlspecialchars(trim($_REQUEST [$val]))) != ''){
			...
			}elseif (($keyword=htmlspecialchars(trim($_REQUEST ['keyword']))) != '') {
				if($is_like){
					if(false!==strpos($keyword,' ')){
						$childstr = ''; $childarr = array();
						$arr = explode(' ', $keyword);
						foreach($arr as $v){
							...
							if($v!=''){
								$childstr .= ($childstr!='' ? '&' : '') . $mapkey;
								$childarr[] = array('like', "%{$v}%");
							}
						}
						if($childarr){
							$childarr['_multi'] = true;
							$map_keyword[$childstr]	= $childarr;
						}
					}else{
						$map_keyword[$mapkey] = array('like', "%{$keyword}%");
					}
				}
			}
		}
		...
		if($map_keyword){
			$map_keyword['_logic'] = 'or';
			$map['_complex'] = $map_keyword;
			$map['_logic'] = 'and';
		}
	}
	return $map;
